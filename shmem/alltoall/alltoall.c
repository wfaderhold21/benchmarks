                ...
                int PE_start, int logPE_stride, int PE_size, long * pSync)
{
    ...
    for (; i < PE_size; i += stride) {
        int peer_index = (i - PE_start) / stride;
        const void *src_chunk = (const char *)source + selems * nelems * peer_index;
        shmem_putmem((void *)((char *) dest + selems * mystarting_index),
                     src_chunk, nelems * selems, i);
    }
    shmem_quiet();
    end = TIME();
...
int main(void) 
{
...
#if WITH_HINTS
    pSync = (long *) shmemx_malloc_with_hint(sizeof(long) * SHMEM_ALLTOALL_SYNC_SIZE, SHMEM_HINT_LOW_LAT_MEM);
    ...
    pWork = (double *) shmem_malloc(sizeof(double) * SHMEM_REDUCE_MIN_WRKDATA_SIZE);
#endif
    pSyncReduce = (long *) shmem_malloc(sizeof(long) * SHMEM_REDUCE_SYNC_SIZE);
    for (i = 0; i < SHMEM_ALLTOALL_SYNC_SIZE; i++) {
        pSync[i] = SHMEM_SYNC_VALUE;
    }
    for (i = 0; i < SHMEM_REDUCE_SYNC_SIZE; i++) {
        pSyncReduce[i] = SHMEM_SYNC_VALUE;
    }
    shmem_barrier_all();
    ...
    for (i = 0; i <= 18; i++) {
...
#if WITH_HINTS
        data = (data_t *) shmemx_malloc_with_hint(sizeof(data_t) * nr_elems * n_pes, SHMEM_HINT_LOW_LAT_MEM);
        alldata = (data_t *) shmemx_malloc_with_hint(sizeof(data_t) * nr_elems * n_pes , SHMEM_HINT_LOW_LAT_MEM);
#else
        data = (data_t *) shmem_malloc(sizeof(data_t) * nr_elems * n_pes);
        alldata = (data_t *) shmem_malloc(sizeof(data_t) * nr_elems * n_pes);
#endif

        for (j = 0; j < nr_elems * n_pes; j++) {
        ...
        size = (1.0 * n_pes * MAX_ITER * nr_elems * sizeof(data_t)); // bytes
        ...
        {   double *src_buff = NULL, *dest_buff = NULL, agg_bw;
			...
			*src_buff = bandwidth;
            shmem_barrier_all();
			shmem_double_sum_to_all(dest_buff, src_buff, 1,0,0,n_pes,pWork,pSyncReduce);
