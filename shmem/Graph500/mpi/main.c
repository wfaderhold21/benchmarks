...
int main(int argc, char** argv) {
  ...
  MPI_Init(&argc, &argv);
    ...
    error = sharp_create_data_tier(&data_tiers, 1, 
                           SHARP_HINT_CPU, SHARP_ACCESS_INTERP, 1); 
//    if(error == 0) {
//        printf("data tier issues\n");
//        return -1;
//    } 
#endif
...
#ifdef WITH_HINTS
  int64_t* pred = (int64_t *) shmemx_malloc_with_hint(nlocalverts * sizeof(int64_t), SHMEM_HINT_NEAR_NIC_MEM);
#else
  int64_t* pred = (int64_t *) shmem_malloc(nlocalverts * sizeof(int64_t));
#endif
